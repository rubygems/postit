...
describe PostIt::Installer do
  ...
  describe '#install!' do
    it "installs bundler" do
      ...
      expect_any_instance_of(Gem::DependencyInstaller).to receive(:install).with('bundler', ruby_version).once
      subject.install!
    end

    it "does not re-install" do
      expect(subject).to receive(:installed?).and_return(true)
      expect_any_instance_of(Gem::DependencyInstaller).to receive(:install).never
      subject.install!
    end
  end
end
